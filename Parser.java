	...
	public void parse() {
        ...
        try {
    		...
    		while (it.hasNext()) {
        		...
        		array = list.subList(1, list.size());
        		Collections.replaceAll(array, "\\[", "");
        		Collections.replaceAll(array, "\\]","");
        		writer.write(list.get(0).toString().replaceAll("\\[", "").replaceAll("\\]","") + "\t" + Arrays.toString(array.toArray()).replace("[", "").replace("]", ""));
        		writer.newLine();
    		}
